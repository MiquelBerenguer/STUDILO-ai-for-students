...
ONLY in the student's own notes. The pack contains a study guide and several realistic mock exams, each
...
4. Mock exams: create exactly the required number of mock exams and questions per exam
   (`draft_question` with mock_exam_number 1..N). Guidelines:
...
6. When every question is verified and the guide is complete, call `save_exam_pack` with a title and
   duration for each mock exam. If it returns problems, fix them and call it again.
