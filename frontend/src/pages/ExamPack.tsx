...
import type { Attempt, ExamPackDetail, PracticeExam, Question } from "../types";
...
export default function ExamPackPage() {
  ...
  const [taking, setTaking] = useState<PracticeExam | null>(null);
  ...
  const p = pack.data;
  if (taking) return <TakeExam practice={taking} cites={p.citations} onExit={() => setTaking(null)} />;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="no-print flex gap-1 border-b">
        {(["guide", "practice"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`-mb-px border-b-2 px-4 py-2 text-sm font-medium ${tab === t ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500"}`}>
            {t === "guide" ? "Study guide" : `Practice exams (${p.practice_exams.length})`}
          </button>
        ))}
      </div>
      ...
      <div className={tab === "practice" ? "space-y-5" : "hidden print:block"}>
        {p.practice_exams.map((m) => (
...
function TakeExam({ practice, cites, onExit }: { practice: PracticeExam; cites: Cites; onExit: () => void }) {
  const [attempt, setAttempt] = useState<Attempt | null>(null);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [scores, setScores] = useState<Record<string, number>>({});
  const [left, setLeft] = useState(practice.duration_minutes * 60);
  const start = useMutation({ mutationFn: () => post<Attempt>(`/practice-exams/${practice.id}/attempts`), onSuccess: setAttempt });
  ...
  const solved = useQuery({
    queryKey: ["practice", practice.id, "reveal"], enabled: Boolean(attempt?.submitted_at),
    queryFn: () => get<PracticeExam>(`/practice-exams/${practice.id}?reveal=true`),
  });
  ...
  const total = practice.questions.reduce((a, q) => a + q.points, 0);
  ...
  return (
    <div className="space-y-4">
      <div className="sticky top-0 z-10 flex items-center justify-between rounded-xl border bg-white p-3 shadow-sm">
        <div><b>{practice.title}</b> <span className="text-sm text-slate-500">· {total} pts</span></div>
      ...
      {!attempt ? (
        <Card>
          <p className="text-sm">Timed mode: {practice.duration_minutes} minutes, {practice.questions.length} questions. Solutions appear after you submit. The exam submits automatically when time runs out.</p>
          <button className="btn-primary mt-3" onClick={() => start.mutate()} disabled={start.isPending}>Start now</button>
          <ErrorBox error={start.error} />
        </Card>
      ) : (
        <ol className="space-y-4">
          {(submitted ? solved.data?.questions ?? practice.questions : practice.questions).map((q, i) => (
